...
int CPNCSysPara::InitPropHashtable()
{
	...
	AddPropItem("m_nMkCirclediameter", PROPLIST_ITEM(id++, "钢印圆直径", "钢印圆直径"));
...
int CPNCSysPara::GetPropValueStr(long id, char* valueStr, UINT nMaxStrBufLen/*=100*/, CPropTreeItem *pItem/*=NULL*/)
{
	...
	else if (GetPropID("m_nMkCirclediameter") == id)
		sText.Printf("%d", m_nMkCirclediameter);
...
BOOL CPNCSysPara::IsHasPlateWeldTag(const char* sText)
{
	...
	if (sContents.Find("焊") >= 0 || sContents.Find("weld") >= 0)
		return TRUE;
	return FALSE;
}
//
BOOL CPNCSysPara::IsHasPlateBendTag(const char* sText)
{
	if (sText == NULL || strlen(sText) <= 0)
		return FALSE;
	CString sContents(sText);
	if (sContents.Find("卷边") >= 0 || sContents.Find("火曲") >= 0 ||
		sContents.Find("外曲") >= 0 || sContents.Find("内曲") >= 0 ||
		sContents.Find("正曲") >= 0 || sContents.Find("反曲") >= 0)
		return TRUE;
	return FALSE;
}
#ifdef __UBOM_ONLY_
...
bool PNCSysSetImportDefault(FILE *fp)
{
	...
	while (!feof(fp))
	{
		...
		else if (_stricmp(key_word, "m_nMkCirclediameter") == 0)
			sscanf(line_txt, "%s%d", key_word, &g_pncSysPara.m_nMkCirclediameter);
...
bool PNCSysSetExportDefault(FILE *fp)
{
	...
	fprintf(fp, "m_nMkCirclediameter=%d ;圆直径\n", g_pncSysPara.m_nMkCirclediameter);
