...
void CHoleIncrementSetDlg::RefreshCtrlState()
{
	...
	{	//切割
		...
		BOOL bEnable = FALSE;
		if (m_ciCurNcMode == CNCPart::FLAME_MODE&&g_sysPara.nc.m_xFlamePara.m_bCutSpecialHole)
			bEnable = TRUE;
		if (m_ciCurNcMode == CNCPart::PLASMA_MODE&&g_sysPara.nc.m_xPlasmaPara.m_bCutSpecialHole)
			bEnable = TRUE;
